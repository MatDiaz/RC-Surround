    ...
    : AudioProcessorEditor (&p), processor (p)
{
    ...
    activeSystemLabel->setFont (Font ("Maiandra GD", 20.00f, Font::plain).withTypefaceStyle ("Regular"));
    ...
    systemLabel->setFont (Font ("Maiandra GD", 20.00f, Font::plain).withTypefaceStyle ("Regular"));
		...
		if (abreCarpeta.browseForDirectory())
		{
			...
			for (int i = 0; i < 6; i++)
			{
				 ...
				 if (readOperator != nullptr)
				 {
					 ...
					 processor.loadImpulseResponseToEngine(localAudioBuffer, i, readOperator->sampleRate);
...
<JUCER_COMPONENT documentType="Component" className="PluginAudioProcessorEditor"
  ...
  <BACKGROUND backgroundColour="ff141414">
  ...
  <LABEL name="activeSystemLabel" id="512db482e2caaf6b" memberName="activeSystemLabel"
         ...
         edTextCol="ff000000" edBkgCol="0" labelText="Sistema Activo"
